...
(defun draw-frame (widget pad)
  (let* ((bt (border-top% widget))      (bt? (plusp bt))
         ...
         (attr *border-attr*))
    (when bl?
      (paint% pad left top 1 th *border-vertical-char* attr)
      (when bt? (put% pad left top    *border-top-left-char*    attr))
      (when bb? (put% pad left bottom *border-bottom-left-char* attr))
      (decf tw)
      (incf left))
    (when br?
      (paint% pad right top 1 th *border-vertical-char* attr)
      (when bt? (put% pad right top    *border-top-right-char*    attr))
      (when bb? (put% pad right bottom *border-bottom-right-char* attr))
      (decf tw)
      (decf right))
    (when bt?
      (paint% pad left top tw 1 *border-horizontal-char* attr)
      (decf th)
      (incf top))
    (when bb?
      (paint% pad left bottom tw 1 *border-horizontal-char* attr)
      (decf th)
      (decf bottom))
    (when fc
